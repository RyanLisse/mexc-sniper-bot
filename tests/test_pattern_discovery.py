...
def mock_calendar_entry():
    ...
    return CalendarEntryApi(
        vcoinId="TEST123",
        symbol="TESTUSDT",
        projectName="Test Token",
        firstOpenTime=future_time
    )
    ...
    async def test_calendar_monitoring(self, discovery_engine, mock_calendar_entry):
        ...
        with patch('src.services.pattern_discovery.get_mexc_client') as mock_client:
            ...
            with patch('src.services.pattern_discovery.get_async_session') as mock_session:
                ...
                with patch('src.database.get_monitored_listing', return_value=None):
                    with patch('src.database.create_monitored_listing') as mock_create:
        ...
        with patch('src.services.pattern_discovery.get_mexc_client') as mock_client:
            ...
            with patch('src.services.pattern_discovery.get_async_session') as mock_session:
                ...
                with patch('src.database.get_monitoring_listings') as mock_get_listings:
                    ...
                    with patch('src.database.get_snipe_target', return_value=None):
                        with patch('src.database.create_snipe_target') as mock_create_target:
    ...
    async def test_full_discovery_flow(self, discovery_engine, mock_calendar_entry, mock_symbol_ready):
        ...
        with patch('src.services.pattern_discovery.get_mexc_client') as mock_client:
            ...
            with patch('src.services.pattern_discovery.get_async_session') as mock_session:
                ...
                with patch('src.database.get_monitored_listing', return_value=None), \
                     patch('src.database.create_monitored_listing') as mock_create_listing, \
                     patch('src.database.get_monitoring_listings') as mock_get_listings, \
                     patch('src.database.get_snipe_target', return_value=None), \
                     patch('src.database.create_snipe_target') as mock_create_target, \
                     patch('src.database.get_pending_snipe_targets', return_value=[]), \
                     patch('src.database.update_snipe_target_status'):
