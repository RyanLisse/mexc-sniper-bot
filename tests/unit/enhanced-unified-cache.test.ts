...
describe('EnhancedUnifiedCacheSystem', () => {
  ...
  describe('TTL Management for Different Data Types', () => {
    ...
    it('should use custom TTL when provided', async () => {
      ...
      await cacheSystem.set('custom-key', customData, 'api_response', customTTL);
  ...
  describe('Batch Operations', () => {
    ...
    it('should handle batch set operations', async () => {
      const entries = [
        { key: 'key1', value: { data: 'value1' }, dataType: 'api_response' as const },
        { key: 'key2', value: { data: 'value2' }, dataType: 'market_data' as const },
        { key: 'key3', value: { data: 'value3' }, dataType: 'pattern_analysis' as const },
      ];
  ...
  describe('Cache Warming', () => {
    ...
    it('should handle warmup strategy failures gracefully', async () => {
      const failingStrategy = vi.fn().mockRejectedValue(new Error('Warmup failed'));
  ...
  describe('Cache Priority Management', () => {
    ...
    it('should assign high priority to pattern analysis', async () => {
      await cacheSystem.set('pattern-key', { confidence: 85 }, 'pattern_analysis');
  ...
  describe('Error Handling', () => {
    ...
    it('should handle cache operation errors gracefully', async () => {
      ...
      await expect(cacheSystem.set('test-key', 'value')).resolves.toBeUndefined();
    });

    it('should handle batch operation errors gracefully', async () => {
      await expect(cacheSystem.mget(['key1', 'key2'])).resolves.toEqual([null, null]);
      await expect(cacheSystem.mset([
  ...
  describe('Integration with Existing Cache System', () => {
    ...
    it('should support all existing data types', async () => {
      const dataTypes = ['generic', 'api_response', 'market_data', 'pattern_analysis', 'trading_signal'] as const;
      
      for (const dataType of dataTypes) {
        await expect(cacheSystem.set(`key-${dataType}`, { data: dataType }, dataType))
          .resolves.toBeUndefined();
      }
    });
  });
