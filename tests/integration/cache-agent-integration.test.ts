...
describe('Cache-Agent Integration', () => {
  ...
  beforeEach(async () => {
    ...
    patternAgent = new TestAgent('pattern-discovery-agent', 'You analyze patterns');
  ...
  describe('Cache Analytics for Agents', () => {
    ...
    test('should provide agent-specific recommendations', async () => {
      ...
      const hasPerformanceRecommendation = analytics.recommendations.some(rec => 
        /cache|hit rate|performance/i.test(rec)
      );
    ...
    test('should monitor cache health for agents', async () => {
      ...
      const cachedHealth = await globalEnhancedAgentCache.getAgentHealth(agentId);
  ...
  describe('Workflow Result Caching', () => {
    test('should cache workflow results', async () => {
      ...
      agentResults.set('pattern-discovery', await patternAgent.process('Analyze patterns'));
      agentResults.set('strategy', await strategyAgent.process('Create strategy'));
      ...
      await globalEnhancedAgentCache.setWorkflowResult(workflowType, parameters, workflowResult);
      
      // Retrieve cached result
      const cached = await globalEnhancedAgentCache.getWorkflowResult(workflowType, parameters);
    ...
    test('should invalidate workflow cache when dependencies change', async () => {
      ...
      // Verify it's cached
      let cached = await globalEnhancedAgentCache.getWorkflowResult(workflowType, parameters);
      expect(cached).toBeTruthy();
      ...
      cached = await globalEnhancedAgentCache.getWorkflowResult(workflowType, parameters);
  ...
  describe('Cache Performance Monitoring', () => {
    ...
    test('should generate cache optimization recommendations', async () => {
      ...
      const hasPerformanceRec = recommendations.some(rec => 
  ...
  describe('Cache Integration with Agent Error Handling', () => {
    test('should handle agent errors without breaking cache', async () => {
      ...
      await expect(errorAgent.process('test input')).rejects.toThrow('API Error');
  ...
  describe('Memory Management', () => {
    test('should manage memory usage effectively', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        for (const agent of agents) {
          promises.push(agent.process(`Request ${i} from ${agent['config'].name}`));
        }
      }
    ...
    test('should cleanup expired cache entries', async () => {
      ...
      await globalCacheManager.set('short-lived-1', { data: '1' }, { ttl: 100 });
      await globalCacheManager.set('short-lived-2', { data: '2' }, { ttl: 100 });
  ...
  describe('Performance Benchmarks', () => {
    test('should demonstrate caching effectiveness', async () => {
      ...
      console.log(`Cache effectiveness: ${cachedResponses.length} consecutive cache hits`);
    });
    ...
    test('should maintain good hit rates under concurrent load', async () => {
      ...
      if (patternPerf) {
        ...
        console.log(`Pattern agent hit rate: ${patternPerf.hitRate.toFixed(1)}%`);
      }
      
      if (strategyPerf) {
        expect(strategyPerf.hitRate).toBeGreaterThan(20);
        expect(strategyPerf.totalRequests).toBeGreaterThan(0);
        console.log(`Strategy agent hit rate: ${strategyPerf.hitRate.toFixed(1)}%`);
      }
    });
  });
});