...
import { TakeProfitStrategy, TAKE_PROFIT_STRATEGIES } from '../../src/types/take-profit-strategies';
...
describe('EnhancedTakeProfitConfig', () => {
  ...
  describe('Preset Strategies', () => {
    ...
    it('should highlight selected strategy', () => {
      render(<EnhancedTakeProfitConfig {...defaultProps} selectedStrategy="conservative" />);

      const conservativeCard = screen.getByText('Conservative').closest('.cursor-pointer');
      expect(conservativeCard).toHaveClass('ring-2', 'ring-primary', 'border-primary');
    });
    ...
    it('should display potential profit calculations', () => {
      render(<EnhancedTakeProfitConfig {...defaultProps} investmentAmount={1000} />);
  ...
  describe('Custom Strategy', () => {
    it('should render custom strategy card', () => {
      ...
      expect(screen.getByText(/Create your own personalized take profit levels/)).toBeInTheDocument();
    });
    ...
    it('should switch to custom tab when custom strategy is selected', async () => {
      ...
      const customCard = screen.getByText('Custom Strategy').closest('.cursor-pointer');
    ...
    it('should display custom levels configuration when custom strategy is selected', () => {
      render(<EnhancedTakeProfitConfig {...defaultProps} selectedStrategy="custom" />);
  ...
  describe('Validation', () => {
    it('should display validation errors for invalid levels', () => {
      ...
      expect(screen.getByText(/Profit percentage must be between/)).toBeInTheDocument();
      expect(screen.getByText(/Sell quantity must be between/)).toBeInTheDocument();
    });
  ...
  describe('Accessibility', () => {
    it('should have proper ARIA labels and roles', () => {
      ...
      expect(screen.getByRole('tab', { name: /Preset Strategies/i })).toBeInTheDocument();
      expect(screen.getByRole('tab', { name: /Custom Strategy/i })).toBeInTheDocument();
    });
  ...
  describe('Performance', () => {
    it('should not re-render unnecessarily', () => {
      const { rerender } = render(<EnhancedTakeProfitConfig {...defaultProps} />);

      // Re-render with same props
      rerender(<EnhancedTakeProfitConfig {...defaultProps} />);

      // Component should handle re-renders gracefully
      expect(screen.getByText('Take Profit Strategy Configuration')).toBeInTheDocument();
    });
