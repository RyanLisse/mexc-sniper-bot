...
export class ApiMockingUtils {
  /**
   * Setup MEXC API mocks for testing
   */
  static setupMexcApiMocks(mexcService: any) {
    // Mock basic market data endpoints
    ...
    vi.spyOn(mexcService, 'getOrderBookDepth').mockImplementation(async (symbol: string) => {
      return MockDataGenerator.generateOrderBook();
    });

    vi.spyOn(mexcService, 'get24hrTicker').mockImplementation(async (symbol: string) => {
      const marketData = MockDataGenerator.generateMarketData('normal');
      return {
        symbol,
        volume: marketData.volume24h.toString(),
        count: Math.floor(Math.random() * 10000),
        high: marketData.high24h.toString(),
        low: marketData.low24h.toString(),
    ...
    vi.spyOn(mexcService, 'placeOrder').mockImplementation(async (symbol, side, amount, price) => {
      return {
        ...
        fee: (amount * price * 0.001).toString()
      };
    });
...
export class AssertionHelpers {
  ...
  static assertOrderExecution(execution: any) {
    ...
    expect(typeof execution.executedQty).toBe('string');
    expect(typeof execution.executedPrice).toBe('string');
    expect(parseFloat(execution.executedQty)).toBeGreaterThan(0);
    expect(parseFloat(execution.executedPrice)).toBeGreaterThan(0);
  }
}
