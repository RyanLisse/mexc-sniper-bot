...
export async function GET(_request: NextRequest) {
  ...
  try {
    console.info(
      "[SystemValidation] Starting comprehensive system validation..."
    );
    ...
    console.info(
      `[SystemValidation] Validation complete. Status: ${validation.overall}, Ready: ${validation.readyForAutoSniping}, Score: ${validation.score}%`
    );
  ...
  try {
    ...
    if (action === "validate_component" && component) {
      ...
      const fullValidation =
        await systemReadinessValidator.validateSystemReadiness();
      ...
      const responseData = {
        ...
        summary: {
          ...
          warnings: componentChecks.filter((c) => c.status === "warning")
            .length,
    ...
    if (action === "quick_health_check") {
      ...
      try {
        ...
        const quickResult = {
          ...
          criticalIssues: validation.checks.filter(
            (c) => c.required && c.status === "fail"
          ).length,
          warnings: validation.checks.filter((c) => c.status === "warning")
            .length,
    ...
    if (action === "get_recommendations") {
      ...
      const responseData = {
        ...
        fixableIssues: validation.checks.filter(
    ...
    return apiResponse.error(
      'Invalid action. Use "validate_component", "quick_health_check", or "get_recommendations"',
      400
    );
  } catch (error) {
