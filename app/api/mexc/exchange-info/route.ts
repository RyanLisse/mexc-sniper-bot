...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!exchangeInfoResponse.success) {
      console.warn("[exchange-info-api] Failed to fetch exchange info:", exchangeInfoResponse.error);
      return apiResponse(
        createErrorResponse(
    ...
    const usdtPairs = exchangeInfoResponse.data?.symbols?.filter(s => s.quoteAsset === "USDT") || [];
    
    console.info(`[exchange-info-api] ✅ Successfully fetched exchange info: ${symbolsCount} total symbols, ${usdtPairs.length} USDT pairs`);
