  ...
  try {
    ...
    if (strategy) {
      ...
      results = await Promise.all(
        strategies.map((strategyName: string) => triggerSingleStrategy(strategyName, force))
      );
    }
...
async function triggerSingleStrategy(strategyName: string, force: boolean = false) {
  try {
    console.log(`[Cache Warming Trigger] Triggering strategy: ${strategyName}`);
...
function getStrategyDescription(strategyName: string): string {
  const descriptions: Record<string, string> = {
    ...
    "pattern-data": "Pre-load pattern detection data for 3.5+ hour advance detection",
  ...
  return descriptions[strategyName] || `Cache warming strategy: ${strategyName}`;
}
