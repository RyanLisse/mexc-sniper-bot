...
function generateMockAlerts(count: number) {
  ...
  const categories = ['system', 'agent', 'trading', 'websocket', 'performance', 'security'];
  const sources = [
    'mexc-api-agent',
  ...
  for (let i = 0; i < count; i++) {
    ...
    alerts.push({
      ...
      acknowledgedAt: Math.random() > 0.5 ? new Date(Date.now() - Math.random() * 24 * 60 * 60 * 1000).toISOString() : null,
      resolved: Math.random() > 0.7,
      resolvedAt: Math.random() > 0.8 ? new Date(Date.now() - Math.random() * 12 * 60 * 60 * 1000).toISOString() : null,
      count: Math.floor(Math.random() * 10 + 1),
      tags: generateAlertTags(category, source),
      metadata: {
        affectedComponents: generateAffectedComponents(category),
        impactLevel: severity === 'critical' ? 'high' : severity === 'error' ? 'medium' : 'low',
...
function generateAlertTitle(severity: string, category: string, source: string): string {
  const titles = {
  ...
  return titles[key as keyof typeof titles] || `${severity.toUpperCase()}: ${category} issue`;
}

function generateAlertMessage(severity: string, category: string, source: string): string {
  const messages = {
    'critical': [
  ...
  const severityMessages = messages[severity as keyof typeof messages] || [`${source} event in ${category}`];
...
function generateAlertTags(category: string, source: string): string[] {
  ...
  return [...baseTags, ...(additionalTags[category as keyof typeof additionalTags] || [])];
}
...
function generateAffectedComponents(category: string): string[] {
  ...
  const categoryComponents = components[category as keyof typeof components] || ['unknown'];
...
async function createAlert(data: any) {
  ...
  console.info(`Creating alert ${alertId}:`, { severity, category, source, title, message });
...
async function bulkAcknowledgeAlerts(data: any) {
  ...
  console.info(`Bulk acknowledging ${alertIds.length} alerts by ${acknowledgedBy}`);
