...
async function healthHandler(request: Request) {
  ...
  try {
    ...
    let authTables: { healthy: boolean; message: string; error: string | null; tables?: Record<string, any> } = { 
      healthy: false, 
    ...
    const healthResult = {
      ...
      details: {
        ...
        optimization: {
          ...
          circuitBreakerActive: (dbHealth as any).fromCircuitBreaker || (authTables as any).fromCircuitBreaker || false
        }
      },
      diagnostics: {
    ...
    const criticalHealthResult = {
      ...
      timestamp: new Date().toISOString(),
      details: {
        database: { healthy: false, message: 'Critical error', error: 'System failure' },
        authTables: { healthy: false, message: 'Critical error', error: 'System failure' },
        environment: {
          AUTH_SECRET: !!process.env.AUTH_SECRET,
          DATABASE_URL: !!process.env.DATABASE_URL,
          DATABASE_URL_PROTOCOL: process.env.DATABASE_URL?.split('://')[0] || 'unknown',
