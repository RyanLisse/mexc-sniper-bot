...
export async function GET() {
  try {
    ...
    const criticalHealthy =
      (strategyHealth.healthy && templateHealth.healthy) ||
      patternHealth.healthy ||
      riskHealth.healthy; // Strategy + templates required, OR other systems working
    const allHealthy =
      ...
      riskHealth.healthy;
    const hasWarnings =
      strategyHealth.warnings ||
      templateHealth.warnings ||
      patternHealth.warnings ||
      riskHealth.warnings;
    ...
    if (!criticalHealthy && isProduction && hasAnyApiKeys) {
      ...
      status = "warning";
      message = hasAnyApiKeys
        ? "Trading strategy system operational with some components degraded"
        : "Trading strategy system running in demonstration mode";
    } else {
      status = "healthy";
...
async function checkTradingStrategies(): Promise<{
  ...
  try {
    ...
    const isHealthy =
      (hasApiKeys && performance.successRate > 70) ||
      (!hasApiKeys && process.env.NODE_ENV !== "production");
    const hasWarnings =
      !hasStrategiesConfig || !hasApiKeys || performance.successRate < 80;
...
async function checkPatternDetection(): Promise<{
  ...
  try {
    ...
    const isHealthy =
      (hasAiKeys && confidenceScore > 60) ||
      (!hasAiKeys && process.env.NODE_ENV !== "production");
...
async function checkRiskManagement(): Promise<{
  ...
  try {
    // Check risk management system configuration
    const hasRiskConfig = !!process.env.RISK_MANAGEMENT_ENABLED;
    const hasEmergencyStops = !!process.env.EMERGENCY_STOP_ENABLED;

    // Basic risk score from environment variable (0-100)
    const riskScore = parseFloat(process.env.RISK_SCORE || "50");

    let currentRiskLevel: "low" | "medium" | "high";
    if (riskScore >= 70) {
    ...
    const stopLossActive = process.env.STOP_LOSS_ACTIVE === "true";
    const positionSizeOptimal = process.env.POSITION_SIZE_SCORE
      ? parseFloat(process.env.POSITION_SIZE_SCORE) >= 70
      : true;

    // Consider healthy in development mode even without emergency stops
    const isHealthy =
      (hasEmergencyStops && stopLossActive) ||
      process.env.NODE_ENV !== "production";

    const hasWarnings =
      !hasRiskConfig || !hasEmergencyStops || currentRiskLevel === "high";
...
async function checkStrategyTemplates(): Promise<{
  ...
  try {
    ...
    if (!health.templatesInitialized && health.databaseConnected) {
      try {
        console.info(
          "[Health Check] Strategy templates not initialized, attempting initialization..."
        );
        await strategyInitializationService.initializeOnStartup();
        // Get updated health status
        const updatedHealth =
          await strategyInitializationService.getHealthStatus();
        return {
          healthy:
            updatedHealth.templatesInitialized &&
            updatedHealth.databaseConnected,
        ...
        console.error(
          "[Health Check] Strategy template initialization failed:",
          { error: initError }
        );
        return {
          healthy: false,
    ...
    console.error("[Health Check] Strategy template check failed:", {
      error: error,
    });
    return {
      healthy: false,
