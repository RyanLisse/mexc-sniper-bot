...
export async function GET(request: NextRequest) {
  try {
    ...
    const detailedMetrics = generateDetailedPerformanceMetrics(timeRange, includeRawData);
 ...
 */
function generateDetailedPerformanceMetrics(timeRange: string, includeRawData: boolean) {
  const now = new Date();
  ...
  for (let i = dataPoints - 1; i >= 0; i--) {
    ...
    metrics.push({
      ...
      successRate: Math.min(1, 0.90 + trend * 0.08 + noise * 0.02),
      ...
      cacheHitRate: Math.min(1, 0.70 + trend * 0.20 + noise * 0.05),
      
      // Performance breakdown by component
      components: {
        patternDetection: {
          responseTime: Math.round(100 + trend * 20 + noise * 15),
          accuracy: Math.min(1, 0.75 + trend * 0.10 + noise * 0.03),
        ...
        riskManagement: {
          ...
          violationRate: Math.max(0, 0.02 - trend * 0.01 + Math.abs(noise) * 0.005),
  ...
  const response: any = {
    ...
    summary: {
      ...
      avgThroughput: Math.round(metrics.reduce((sum, m) => sum + m.throughput, 0) / metrics.length * 10) / 10,
      avgErrorRate: Math.round(metrics.reduce((sum, m) => sum + m.errorRate, 0) / metrics.length * 1000) / 1000,
      avgSuccessRate: Math.round(metrics.reduce((sum, m) => sum + m.successRate, 0) / metrics.length * 1000) / 1000,
