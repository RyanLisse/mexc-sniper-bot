  ...
  try {
    ...
    if (!name || !description || !parameters || !variants) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: name, description, parameters, variants",
        },
        { status: 400 }
      );
    }
...
function generateABTestData(
  ...
  const _statuses = ["running", "completed", "failed", "paused"] as const;
  const now = new Date();
  ...
  const tests = [
    {
      ...
      description:
        "Testing different take profit strategies to maximize profitability while minimizing risk",
      ...
      variants: [
        ...
        },
        {
          ...
          description:
            "Higher take profit percentage with moderate success rate",
...

    {
      ...
      description:
        "Optimizing pattern detection sensitivity to reduce false positives while maintaining accuracy",
      status: "running" as const,
      startDate: new Date(
...

    {
      ...
      description:
        "Testing adaptive risk management that adjusts to market volatility",
...

    {
      ...
      description:
        "Testing different entry timing strategies for pattern-based trades",
      ...
      failureReason:
        "Insufficient statistical significance - extending test duration required",
    },
  ];
