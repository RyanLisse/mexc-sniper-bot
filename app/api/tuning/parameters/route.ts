...
export async function GET(request: NextRequest) {
  try {
    ...
    if (includeDefinitions) {
      const definitions = Object.fromEntries(parameterManager.getParameterDefinitions());
  ...
  try {
    ...
    switch (action) {
      case 'validate': {
        ...
        const validation = await parameterManager.validateParameters(parameters);
      ...
      case 'restore_snapshot': {
        ...
        logger.info('Parameters restored from snapshot via API', { snapshotId });
