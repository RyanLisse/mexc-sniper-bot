...
export async function GET(request: NextRequest) {
  try {
    ...
    const constraints = generateSafetyConstraints(category, status, severity, includeDisabled);
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        violations: constraints.filter(c => c.currentStatus === 'violation').length,
        warnings: constraints.filter(c => c.currentStatus === 'warning').length,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, description, type, category, severity, value, validation } = body;

    // Validate required fields
    if (!name || !description || !type || !category || !severity || value === undefined) {
      return NextResponse.json(
...
function generateSafetyConstraints(
  ...
  const constraints = [
    {
      ...
      description: 'Limit maximum position size to prevent overexposure and maintain portfolio diversification',
...

    {
      ...
      description: 'Limit number of trades per day to prevent overtrading and preserve capital',
      type: 'threshold' as const,
      category: 'risk' as const,
      ...
      violationCount: 3,
      metadata: {
        lastViolation: new Date(now.getTime() - 6 * 60 * 60 * 1000).toISOString(), // 6 hours ago
...

    {
      ...
      description: 'Stop trading when portfolio drawdown exceeds acceptable threshold',
      type: 'threshold' as const,
      category: 'risk' as const,
      severity: 'critical' as const,
      enabled: true,
...

    {
      ...
      description: 'Ensure sufficient liquidity before entering positions to avoid slippage',
      type: 'threshold' as const,
      ...
      metadata: {
        ...
        lastLowLiquidityEvent: new Date(now.getTime() - 12 * 60 * 60 * 1000).toISOString()
      }
    },

    {
      ...
      description: 'Minimum confidence level required for pattern-based trading signals',
      type: 'threshold' as const,
      category: 'performance' as const,
      severity: 'medium' as const,
      enabled: true,
...

    {
      ...
      description: 'Maximum acceptable system response time for trade execution',
      type: 'threshold' as const,
...

    {
      ...
      description: 'Maintain buffer below exchange API rate limits to prevent throttling',
      type: 'threshold' as const,
      category: 'system' as const,
      severity: 'medium' as const,
      enabled: true,
      locked: false,
      value: 0.80, // Use max 80% of rate limit
      defaultValue: 0.75,
      validation: { min: 0.50, max: 0.95 },
      currentStatus: 'ok' as const,
      ...
      metadata: {
        ...
        peakUsage: 0.89,
        rateLimitHits: 2,
        lastThrottling: new Date(now.getTime() - 3 * 60 * 60 * 1000).toISOString()
      }
    },

    {
      ...
      description: 'Automatic emergency stop when multiple violations occur simultaneously',
      ...
      metadata: {
        triggerCount: 1,
        lastTrigger: new Date(now.getTime() - 48 * 60 * 60 * 1000).toISOString(), // 2 days ago
...

    {
      ...
      description: 'Prevent trading when account balance falls below minimum threshold',
      type: 'threshold' as const,
      category: 'risk' as const,
      severity: 'critical' as const,
  ...
  if (categoryFilter) {
    filteredConstraints = filteredConstraints.filter(c => c.category === categoryFilter);
  }

  if (statusFilter) {
    filteredConstraints = filteredConstraints.filter(c => c.currentStatus === statusFilter);
  }

  if (severityFilter) {
    filteredConstraints = filteredConstraints.filter(c => c.severity === severityFilter);
  }
