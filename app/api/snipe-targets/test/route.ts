  ...
  try {
    ...
    const sampleTargets = [
      ...
      },
      {
        ...
        confidenceScore: 67,
        riskLevel: 'low',
        targetExecutionTime: Math.floor((Date.now() + 2 * 60 * 60 * 1000) / 1000), // 2 hours from now
        createdAt: Math.floor(Date.now() / 1000),
        updatedAt: Math.floor(Date.now() / 1000),
      }
    ];
    ...
    const results = await db.insert(snipeTargets).values(sampleTargets).returning();
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const deleted = await db.delete(snipeTargets)
