...
import { SafetyMonitorAgent, type AgentBehaviorMetrics, type SafetyProtocolViolation, type AgentConsensusRequest } from '../mexc-agents/safety-monitor-agent';
...
export interface SafetyAlert {
  id: string;
  type: 'agent_anomaly' | 'risk_breach' | 'emergency_condition' | 'consensus_failure' | 'system_degradation';
...
export class ComprehensiveSafetyCoordinator extends EventEmitter {
  ...
  constructor(config: Partial<SafetyCoordinatorConfig> = {}) {
    ...
    console.log('[ComprehensiveSafetyCoordinator] Initialized with comprehensive safety monitoring');
  }
  ...
  private async performAgentMonitoring(): Promise<void> {
    try {
      ...
      this.currentStatus.agents = {
        ...
        degradedCount: mockMetrics.filter(m => m.anomalyScore >= 30 && m.anomalyScore < 60).length,
        criticalCount: mockMetrics.filter(m => m.anomalyScore >= 60).length,
        offlineCount: 0,
        averagePerformance: mockMetrics.reduce((sum, m) => sum + m.successRate, 0) / mockMetrics.length,
  ...
  private async performSystemHealthCheck(): Promise<void> {
    try {
      ...
      this.currentStatus.emergency = {
        ...
        emergencyLevel: systemHealth.overall === 'critical' ? 'critical' : 
                       systemHealth.overall === 'degraded' ? 'medium' : 'none',
      };
  ...
  private async updateSafetyStatus(): Promise<void> {
    ...
    safetyScore -= Math.max(0, this.currentStatus.risk.overallRiskScore - this.config.riskScoreThreshold);
  ...
  private async createAlert(alertData: Omit<SafetyAlert, 'id' | 'timestamp' | 'acknowledged' | 'resolved'>): Promise<string> {
  ...
  private async recordSafetyAction(actionData: Omit<SafetyAction, 'id' | 'executedAt'>): Promise<void> {
    ...
    console.log(`[ComprehensiveSafetyCoordinator] Executed ${action.type} action: ${action.reason}`);
  }
