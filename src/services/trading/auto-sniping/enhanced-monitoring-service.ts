...
export class EnhancedMonitoringService extends EventEmitter {
  ...
  logExecutionStart(target: AutoSnipeTarget): string {
    ...
    this.addExecutionLog(
      "execution_start",
      target.symbolName,
      "Auto-sniping execution started",
      {
        id: logId,
        targetId: target.id,
  ...
  logExecutionComplete(
    ...
    this.addExecutionLog(
      "execution_complete",
      symbol,
      "Auto-sniping execution completed",
      {
        logId,
  ...
  getCurrentMetrics(): PerformanceMetrics {
    ...
    const recentLogs = this.executionLogs.filter(
      (log) => now - log.timestamp < 3600000 // Last hour
    );
    ...
    const successful = executions.filter(
      (log) => log.type === "execution_complete"
    );
  ...
  private startMetricsCollection(): void {
    ...
    const collectMetrics = () => {
      ...
      const cutoff =
        Date.now() - this.config.retentionPolicy.keepMetricsForHours * 3600000;
  ...
  private startLogCleanup(): void {
    setInterval(() => {
      const cutoff =
        Date.now() - this.config.retentionPolicy.keepLogsForHours * 3600000;
  ...
  private checkExecutionTimeAlert(executionTime: number): void {
    ...
    if (executionTime > this.config.alertThresholds.executionTimeMs) {
      this.createAlert(
        ...
        {
          executionTime,
          threshold: this.config.alertThresholds.executionTimeMs,
        }
      );
    }
  }
  ...
  private checkCircuitBreakerAlert(): void {
    ...
    if (
      this.circuitBreakerTrips >=
      this.config.alertThresholds.circuitBreakerTrips
    ) {
      this.createAlert(
        "system",
        "critical",
  ...
  private checkMetricsAlerts(metrics: PerformanceMetrics): void {
    ...
    if (
      metrics.successRate <
      100 - this.config.alertThresholds.failureRatePercent
    ) {
      this.createAlert(
        "performance",
        "high",
  ...
  private countActivePositions(): number {
    ...
    try {
      ...
      const openPositionLogs = this.executionLogs.filter(
        (log) =>
          log.type === "position_opened" &&
          !this.executionLogs.some(
            (closeLog) =>
      ...
      this.logger.debug("Unable to get accurate position count", {
  ...
  private calculateRealizedPnL(): number {
    ...
    try {
      ...
      const closedPositionLogs = this.executionLogs.filter(
        (log) =>
      ...
      const tradeCompleteLogs = this.executionLogs.filter(
        (log) =>
      ...
      this.logger.warn("Failed to calculate realized PnL", {
        error: toSafeError(error).message,
      });
    }
  ...
  private calculateUnrealizedPnL(): number {
    ...
    try {
      ...
      this.logger.warn("Failed to calculate unrealized PnL", {
        error: toSafeError(error).message,
      });
    }
