...
import { patternDetectionEngine, type PatternAnalysisResult, type PatternMatch } from "./pattern-detection-engine";
...
import { monitoredListings, patternEmbeddings } from "@/src/db/schemas/patterns";
...
export class PatternStrategyOrchestrator {
  ...
  private async executeDiscoveryWorkflow(
    ...
    const monitoringPlan = await this.createMonitoringPlan(
      patternAnalysis.matches.filter(m => 
  ...
  private async executeMonitoringWorkflow(
    ...
    if (symbolData.length > 0) {
      ...
      if (request.options?.enableAgentAnalysis) {
        const readyCandidates = patternAnalysis.matches.filter(m => 
          m.patternType === "ready_state" || 
          (m.patternType === "pre_ready" && m.confidence >= 75)
        );

        for (const candidate of readyCandidates.slice(0, 3)) { // Limit to top 3
  ...
  private async executeValidationWorkflow(
    ...
    if (request.input.symbolData) {
      ...
      const readyPatterns = patternAnalysis.matches.filter(m => 
        m.patternType === "ready_state"
      );
      ...
      for (const pattern of readyPatterns) {
        const validationResponse = await this.patternAgent.validateReadyState({
          ...
          symbolData: request.input.symbolData!.filter(s => 
            s.cd === pattern.symbol
          ),
          count: 1
        });
  ...
  private generateReasoning(pattern: PatternMatch): string {
    ...
    if (pattern.patternType === "ready_state") {
      reasons.push(`Ready state pattern detected (sts:${pattern.indicators.sts}, st:${pattern.indicators.st}, tt:${pattern.indicators.tt})`);
    }
  ...
  private calculateOptimalTiming(pattern: PatternMatch): StrategicRecommendation["timing"] {
    ...
    if (pattern.patternType === "ready_state") {
      ...
      timing.monitoringStart = new Date(now.getTime() + Math.max((launchHours - 1) * 60 * 60 * 1000, 0));
    }

    return timing;
  }

  private calculateRiskManagement(pattern: PatternMatch): StrategicRecommendation["riskManagement"] {
    const baseRisk = pattern.riskLevel === "low" ? 0.02 : pattern.riskLevel === "medium" ? 0.03 : 0.05;
    ...
    return {
      ...
      takeProfit: pattern.patternType === "ready_state" ? 1.15 : 1.10 // 10-15% take profit
    };
  }
  ...
  private async createMonitoringPlan(patterns: PatternMatch[]): Promise<MonitoringPlan> {
    ...
    for (const pattern of patterns) {
      ...
      targets.push({
        ...
        expectedReadyTime: pattern.patternType === "launch_sequence" 
  ...
  private async storeDiscoveredPatterns(patterns: PatternMatch[]): Promise<void> {
    try {
      for (const pattern of patterns) {
        if ((pattern.patternType === "launch_sequence" || pattern.patternType === "ready_state") && pattern.advanceNoticeHours >= 3.5) {
          // Store in monitored listings for tracking
          await db.insert(monitoredListings).values({
            vcoinId: pattern.vcoinId || pattern.symbol,
