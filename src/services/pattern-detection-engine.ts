...
import type { PatternEmbedding, NewPatternEmbedding } from "@/src/db/schemas/patterns";
import { patternEmbeddingService, type PatternData } from "./pattern-embedding-service";
import { db } from "@/src/db";
import { patternEmbeddings, monitoredListings } from "@/src/db/schemas/patterns";
...
export class PatternDetectionEngine {
  ...
  async detectReadyStatePattern(
    ...
    console.log(`[PatternDetectionEngine] Detected ${matches.length} ready state patterns in ${Date.now() - startTime}ms`);
    return matches;
  }
  ...
  async detectAdvanceOpportunities(
    ...
    console.log(`[PatternDetectionEngine] Detected ${matches.length} advance opportunities in ${Date.now() - startTime}ms`);
    return matches;
  }
  ...
  async analyzePatterns(request: PatternAnalysisRequest): Promise<PatternAnalysisResult> {
    ...
    console.log(`[PatternDetectionEngine] Analysis completed: ${filteredMatches.length}/${allMatches.length} patterns above threshold in ${executionTime}ms`);
  ...
  private validateExactReadyState(symbol: SymbolEntry): boolean {
    return symbol.sts === this.READY_STATE_PATTERN.sts &&
  ...
  private async analyzeLaunchTimingCorrelations(symbols: SymbolEntry[]): Promise<CorrelationAnalysis> {
    ...
    return {
      ...
      recommendations: statusPattern > 0.7 ? 
  ...
  private calculateSummary(allMatches: PatternMatch[], filteredMatches: PatternMatch[]) {
    ...
    const avgConfidence = filteredMatches.length > 0 
  ...
  private async getHistoricalSuccessRate(patternType: string): Promise<number> {
    try {
      ...
      const totalAttempts = patterns.reduce((sum, p) => sum + p.truePositives + p.falsePositives, 0);
