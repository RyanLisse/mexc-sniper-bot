...
export class BacktestingFramework extends EventEmitter {
  ...
  private async runSimulation(
    ...
    for (let i = 1; i < marketData.length; i++) {
      ...
      const patternSignal = this.evaluatePatternSignal(
        currentBar,
        previousBar,
        config.parameters
      );
      
      if (patternSignal.signal === 'buy' && patternSignal.confidence >= config.parameters.pattern_confidence_threshold) {
      ...
      for (const position of openPositions) {
        const exitSignal = this.evaluateExitSignal(
          position,
          currentBar,
          config.parameters
        );
  ...
  private evaluateExitSignal(
    ...
    const holdTime = (currentBar.timestamp.getTime() - position.entryTime.getTime()) / (1000 * 60 * 60);
  ...
  private calculatePerformance(
    ...
    const averageWin = winningTrades.length > 0 ? winningTrades.reduce((sum, r) => sum + r, 0) / winningTrades.length : 0;
    const averageLoss = losingTrades.length > 0 ? Math.abs(losingTrades.reduce((sum, r) => sum + r, 0) / losingTrades.length) : 0;
    ...
    return {
      ...
      treynorRatio: this.calculateTreynorRatio(annualizedReturn, this.calculateBeta(dailyReturns, marketData)),
  ...
  private calculateDailyReturns(trades: Trade[], marketData: MarketData[]): number[] {
    ...
    return marketData.slice(1).map((bar, i) => 
      (bar.close - marketData[i].close) / marketData[i].close
    );
  }
  ...
  private generateEquityCurve(trades: Trade[], marketData: MarketData[]): Array<{ timestamp: Date; value: number }> {
    ...
    for (const bar of marketData) {
      ...
      const tradesAtTime = trades.filter(t => 
        t.exitTime && t.exitTime.getTime() === bar.timestamp.getTime()
      );
  ...
  private generateDrawdownCurve(equityCurve: Array<{ timestamp: Date; value: number }>): Array<{ timestamp: Date; drawdown: number }> {
  ...
  private calculateMonthlyReturns(equityCurve: Array<{ timestamp: Date; value: number }>): Array<{ month: string; return: number }> {
  ...
  private calculateRiskMetrics(performance: BacktestPerformance, trades: Trade[], marketData: MarketData[]): Record<string, number> {
    return {
  ...
  private async runScenarioAnalysis(config: BacktestConfig, trades: Trade[]): Promise<Record<string, any>> {
  ...
  private validateResults(performance: BacktestPerformance, config: BacktestConfig): { passed: boolean; violations: string[]; warnings: string[] } {
    const violations: string[] = [];
    const warnings: string[] = [];
    
    // Check risk limits
    if (config.riskLimits) {
      if (config.riskLimits.maxDrawdown && performance.maxDrawdown > config.riskLimits.maxDrawdown) {
        violations.push(`Max drawdown ${performance.maxDrawdown} exceeds limit ${config.riskLimits.maxDrawdown}`);
      }
      
      if (config.riskLimits.minSharpeRatio && performance.sharpeRatio < config.riskLimits.minSharpeRatio) {
        violations.push(`Sharpe ratio ${performance.sharpeRatio} below minimum ${config.riskLimits.minSharpeRatio}`);
      }
    }
  ...
  private calculateTailRatio(trades: Trade[]): number {
    ...
    const avgTopGains = positiveReturns.sort((a, b) => b - a).slice(0, top10Percent).reduce((sum, r) => sum + r, 0) / top10Percent;
    const avgBottomLosses = Math.abs(negativeReturns.sort((a, b) => a - b).slice(0, bottom10Percent).reduce((sum, r) => sum + r, 0) / bottom10Percent);
  ...
  private calculateBurkeRatio(performance: BacktestPerformance, trades: Trade[]): number {
    ...
    return performance.maxDrawdown > 0 ? performance.annualizedReturn / Math.sqrt(performance.maxDrawdown) : 0;
  }
  ...
  closePosition(position: Trade): void {
    ...
    if (index !== -1) {
      ...
      if (position.exitPrice && position.pnl !== undefined) {
        this.portfolioValue += position.quantity * position.exitPrice - position.fees + position.pnl;
      }
    }
  }
