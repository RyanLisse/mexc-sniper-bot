...
class WebSocketMemoryTest {
  ...
  async testLongRunningSubscriptions(): Promise<void> {
    ...
    this.testInterval = setInterval(() => {
      ...
      console.log(`Progress: ${progress}% - Memory: ${this.formatBytes(this.metrics.currentMemory)}`);
  ...
  async testMemoryCleanup(): Promise<void> {
    ...
    const recoveryPercentage = (memoryRecovered / (memoryAfterHeavyUse - this.metrics.startMemory)) * 100;
