...
export class ABTestingFramework extends EventEmitter {
  ...
  async analyzeTest(testId: string): Promise<ABTestResult> {
    ...
    const effectSize = this.calculateCohenD(
      treatmentMean, controlMean, treatmentStdDev, controlStdDev,
      treatmentMetrics.length, controlMetrics.length
    );

    // Statistical significance test (Welch's t-test)
    const tTestResult = this.welchTTest(
      controlMetrics, treatmentMetrics,
      controlMean, treatmentMean, controlStdDev, treatmentStdDev
    );

    const pValue = tTestResult.pValue;
    const confidenceInterval = this.calculateConfidenceInterval(
      effect, tTestResult.standardError, tTestResult.degreesOfFreedom, config.significanceLevel
    );

    // Statistical power analysis
    const statisticalPower = this.calculateStatisticalPower(
      effectSize, controlMetrics.length, treatmentMetrics.length, config.significanceLevel
    );

    const requiredSampleSize = this.calculateRequiredSampleSize(
      effectSize, config.significanceLevel, 0.8 // 80% power
    );
    ...
    const recommendations = this.generateRecommendations(
      pValue, effect, effectSize, statisticalPower, 
      controlMetrics.length + treatmentMetrics.length,
      requiredSampleSize, adjustedAlpha
    );
  ...
  private generateRecommendations(
    ...
    requiredSampleSize: number,
    alpha: number
  ): { decision: 'adopt' | 'reject' | 'extend_test' | 'inconclusive'; confidence: number; reasoning: string[] } {
    const reasoning: string[] = [];
    let decision: 'adopt' | 'reject' | 'extend_test' | 'inconclusive';
    let confidence: number;

    if (pValue < alpha && effect > 0) {
      if (effectSize > 0.2) { // Small to medium effect
        decision = 'adopt';
        confidence = 0.9;
  ...
  private performBayesianAnalysis(
    ...
    for (let i = 0; i < numSamples; i++) {
      ...
      const controlSample = this.normalRandom(controlMean, controlStd / Math.sqrt(controlMetrics.length));
      const treatmentSample = this.normalRandom(treatmentMean, treatmentStd / Math.sqrt(treatmentMetrics.length));
  ...
  assignUser(
    ...
    if (this.participants.has(userId)) {
      ...
      return {
        ...
        parameters: participant.group === 'control' 
