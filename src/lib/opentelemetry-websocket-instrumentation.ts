...
class WebSocketInstrumentation {
  ...
  traceMessage(connectionId: string, direction: 'inbound' | 'outbound', messageType: string, content?: any): void {
...
export function initializeWebSocketInstrumentation(config?: Partial<WebSocketInstrumentationConfig>): void {
  webSocketInstrumentation.initialize(config);
}

export function instrumentChannelOperation(channel: string, operation: string, metadata?: any): void {
