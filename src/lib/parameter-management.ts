...
export class ParameterManager extends EventEmitter {
   ...
   */
  async updateParameter(name: string, value: any, metadata?: Record<string, any>): Promise<void> {
    ...
    logger.info(`Parameter '${name}' updated`, {
      oldValue,
      newValue: value,
  ...
  async updateParameters(
    ...
    if (restartRequired) {
      const affectedComponents = changes.flatMap(
        (change) => this.parameterDefinitions.get(change.parameter)?.affectedComponents || []
      );

      this.emit("restartRequired", {
        parameters: changes.map((c) => c.parameter),
  ...
  private async validateParameterDependencies(
    ...
    if (parameters.pattern_confidence_threshold && parameters.advance_detection_hours) {
      if (parameters.pattern_confidence_threshold > 0.9 && parameters.advance_detection_hours < 2) {
        warnings.push(
          "High confidence threshold with short advance detection may reduce opportunities"
        );
      }
    }
  }
  ...
  async restoreFromSnapshot(snapshotId: string): Promise<void> {
    ...
    await this.createSnapshot(
      `backup_before_restore_${snapshotId}`,
