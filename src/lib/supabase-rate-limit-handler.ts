...
export interface RateLimitInfo {
  ...
  retryAfter?: number;
  limitType?: 'email' | 'otp' | 'verification' | 'token_refresh' | 'mfa' | 'anonymous' | 'api' | 'database' | 'realtime';
  message: string;
  suggestion?: string;
...
export class SupabaseRateLimitHandler {
  ...
  static isRateLimitError(error: any): boolean {
    ...
    const hasRateLimitError = this.RATE_LIMIT_ERRORS.some(pattern => 
    ...
    const hasRateLimitKeywords = [
      'rate', 'limit', 'too many', 'throttle', 'quota', 'exceeded',
      'temporary', 'retry', 'wait', 'slow down', 'congestion'
    ...
    const hasSupabaseRateLimit = [
      'supabase', 'gotrue', 'auth', 'realtime', 'database'
    ].some(service => message.includes(service)) && hasRateLimitKeywords;
  ...
  static analyzeRateLimitError(error: any): RateLimitInfo {
    ...
    for (const [type, pattern] of Object.entries(this.LIMIT_PATTERNS)) {
    ...
    const severity = this.determineSeverity(limitType, retryAfter);

    // Update metrics by type
    if (!this.metrics.rateLimitsByType[limitType]) {
      this.metrics.rateLimitsByType[limitType] = 0;
    }
    this.metrics.rateLimitsByType[limitType]++;
    ...
    const rateLimitInfo: RateLimitInfo = {
      ...
      retryAfter,
      limitType,
      message: this.getFriendlyMessage(limitType, retryAfter),
      suggestion: this.getSuggestion(limitType),
      severity,
      errorCode: code || this.mapErrorCode(error),
      timestamp: Date.now(),
      requestId,
      metadata: {
    ...
    this.logger.error('Rate limit analysis complete', rateLimitInfo);
   ...
   */
  private static determineSeverity(limitType: string, retryAfter?: number): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private static mapErrorCode(error: any): string {
    ...
    if (message.includes('email') && message.includes('rate')) {
      return this.ERROR_CODES.EMAIL_RATE_LIMIT;
    }
    if (message.includes('too many')) {
      return this.ERROR_CODES.TOO_MANY_REQUESTS;
    }
    ...
    if (message.includes('session not found')) {
      return this.ERROR_CODES.SESSION_NOT_FOUND;
    }
   ...
   */
  private static getFriendlyMessage(limitType?: RateLimitInfo['limitType'], retryAfter?: number): string {
   ...
   */
  static calculateBackoffDelay(attempt: number, config: RetryConfig = this.DEFAULT_RETRY_CONFIG): number {
    ...
    const jitter = config.enableJitter ? 
      Math.random() * exponentialDelay * 0.1 : 0;

    // Adaptive retry: increase delay based on recent failures
    let adaptiveMultiplier = 1;
    if (config.adaptiveRetry) {
      const recentFailures = this.metrics.rateLimitedRequests;
      const recentSuccess = this.metrics.successfulRetries;
    ...
    this.logger.debug(`Calculated backoff delay: ${finalDelay}ms`, {
      attempt,
      exponentialDelay,
      jitter,
      adaptiveMultiplier,
  ...
  static recordCircuitBreakerFailure(): void {
    ...
    if (this.circuitBreaker.failureCount >= failureThreshold &&
      ...
      this.circuitBreaker.nextAttemptTime = now + (60000 * Math.pow(2, Math.min(this.circuitBreaker.failureCount - failureThreshold, 4))); // Max 16 minutes
   ...
   */
  static shouldRetry(rateLimitInfo: RateLimitInfo, attempt: number, config: RetryConfig = this.DEFAULT_RETRY_CONFIG): boolean {
    if (!rateLimitInfo.isRateLimited) return false;
    
    // Check circuit breaker first
    if (config.circuitBreakerEnabled && this.isCircuitBreakerOpen()) {
    ...
    if (rateLimitInfo.limitType === 'email') {
      this.logger.debug('Email rate limit - no retry');
      return false;
    }
    
    // Don't retry critical severity errors for too long
    if (rateLimitInfo.severity === 'critical' && attempt > 1) {
      this.logger.debug('Critical severity - limited retries');
      return false;
    }
    ...
    if (rateLimitInfo.retryAfter && rateLimitInfo.retryAfter > config.maxDelay / 1000) {
      this.logger.debug('Retry delay too long');
      return false;
    }
...
export async function withRateLimitHandling<T>(
  ...
  for (let attempt = 0; attempt < config.maxRetries; attempt++) {
    try {
      ...
      SupabaseRateLimitHandler.logger.info(`Retrying operation (attempt ${attempt + 1}/${config.maxRetries}) after ${delay}ms delay`, {
        rateLimitInfo,
        attempt,
        delay,
 ...
 */
export async function bypassRateLimitInDev(email: string, strategy: 'email' | 'admin' | 'direct' = 'email'): Promise<boolean> {
  if (!SupabaseRateLimitHandler.canBypassInDevelopment()) {
    SupabaseRateLimitHandler.logger.warn('Rate limit bypass is only available in development with bypass flag enabled');
    return false;
  }
  ...
  try {
    ...
    if (success) {
      SupabaseRateLimitHandler.logger.info(`✅ Successfully bypassed rate limit for: ${email} using strategy: ${strategy}`);
    } else {
      SupabaseRateLimitHandler.logger.error(`❌ Failed to bypass rate limit for: ${email} using strategy: ${strategy}`);
    }
...
export function getRateLimitStatus(): {
  ...
  if (failureRate > 0.1) {
    recommendations.push('High rate limit failure rate detected - consider implementing request throttling');
  }
