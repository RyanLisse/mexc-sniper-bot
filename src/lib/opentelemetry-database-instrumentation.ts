...
export async function instrumentPatternQuery<T>(
  ...
  operation: () => Promise<T>,
  additionalData?: { symbolCount?: number; patternType?: string }
): Promise<T> {
  return instrumentDatabaseQuery(
    `pattern_${queryType}`,
    operation,
    {
      operationType: queryType === 'search' || queryType === 'analyze' ? 'select' : 
...
export function instrumentDatabase(db: any) {
  return new Proxy(db, {
    get(target, prop) {
      ...
      if (typeof originalValue === 'function' && ['query', 'execute', 'transaction'].includes(prop as string)) {
...
function detectQueryType(query: string): DatabaseSpanOptions['operationType'] {
  ...
  if (normalizedQuery.startsWith('begin') || normalizedQuery.startsWith('commit')) return 'transaction';
