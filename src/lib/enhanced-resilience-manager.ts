...
export class EnhancedCircuitBreaker extends EventEmitter {
    ...
    switch (this.state) {
      case CircuitState.OPEN:
        if (this.nextAttempt && now >= this.nextAttempt) {
          ...
          this.emit('state-change', { name: this.name, from: CircuitState.OPEN, to: CircuitState.HALF_OPEN })
          return false
        }
        return true
  ...
  private onSuccess(responseTime: number): void {
    ...
    switch (this.state) {
      case CircuitState.HALF_OPEN:
        if (this.successCount >= this.config.successThreshold) {
          ...
          this.failureCount = 0
          this.emit('state-change', { name: this.name, from: CircuitState.HALF_OPEN, to: CircuitState.CLOSED })
        }
        break
    ...
    this.emit('success', { name: this.name, responseTime, successCount: this.successCount })
  }
    ...
    switch (this.state) {
      case CircuitState.CLOSED:
        if (this.failureCount >= this.config.failureThreshold) {
          ...
          this.emit('state-change', { name: this.name, from: CircuitState.CLOSED, to: CircuitState.OPEN })
        }
        break
      ...
      case CircuitState.HALF_OPEN:
        this.state = CircuitState.OPEN
        this.nextAttempt = Date.now() + this.config.resetTimeout
        this.emit('state-change', { name: this.name, from: CircuitState.HALF_OPEN, to: CircuitState.OPEN })
        break
    }
...
export class ResilienceCoordinator {
  ...
  getOrCreateCircuitBreaker(name: string, config?: Partial<CircuitBreakerConfig>): EnhancedCircuitBreaker {
    if (!this.circuitBreakers.has(name)) {
      ...
      circuitBreaker.on('state-change', (event) => {
        console.log(`[Resilience] Circuit breaker [${event.name}] state changed: ${event.from} → ${event.to}`)
      })
  ...
  getSystemResilienceMetrics(): {
    ...
    const healthyCount = circuitBreakerMetrics.filter(cb => cb.state === CircuitState.CLOSED).length
    const avgSuccessRate = circuitBreakerMetrics.reduce((sum, cb) => sum + cb.metrics.successRate, 0) / totalCbs
    ...
    if (openCircuits.length > 0) {
      recommendations.push(`${openCircuits.length} circuit breaker(s) are OPEN: ${openCircuits.map(cb => cb.name).join(', ')}`)
    }
    ...
    if (overallHealth < 0.8) {
      recommendations.push('Overall system resilience is below 80% - consider implementing additional fallback strategies')
    }
...
export function getSystemResilienceStatus(): {
  ...
  const openCircuitCount = metrics.circuitBreakers.filter(cb => cb.state === CircuitState.OPEN).length
