...
export function calculateOptimizedPatternSimilarity(pattern1: any, pattern2: any): number {
  try {
    ...
    const data2 = typeof pattern2.data === 'string' ? JSON.parse(pattern2.data) : (pattern2.data || {});
...
export function estimateOptimizedMemoryUsage(cache: Map<string, any>): number {
  ...
  for (const [key, value] of cache.entries()) {
    ...
    if (value && value.data) {
      memoryUsage += typeof value.data === 'string' 
        ? value.data.length * 2 
        : 100; // Estimated object overhead
    }
