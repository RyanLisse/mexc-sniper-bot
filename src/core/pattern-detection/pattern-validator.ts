...
export class PatternValidator implements IPatternValidator {
  ...
  validateCalendarEntry(entry: CalendarEntry): {
    ...
    try {
      ...
      // Optional fields validation
      if (entry.vcoinId !== undefined && (typeof entry.vcoinId !== 'string' || entry.vcoinId.length === 0)) {
        warnings.push('Vcoin ID should be a non-empty string if provided');
      }

      if (entry.projectName !== undefined && (typeof entry.projectName !== 'string' || entry.projectName.length === 0)) {
  ...
  validatePatternMatch(match: PatternMatch): {
    ...
    try {
      ...
      if (!match.recommendation || !['immediate_action', 'monitor_closely', 'prepare_entry', 'wait', 'avoid'].includes(match.recommendation)) {
        errors.push('Recommendation must be one of: immediate_action, monitor_closely, prepare_entry, wait, avoid');
      }
      ...
      if (typeof match.advanceNoticeHours === 'number') {
        ...
        if (match.advanceNoticeHours > 8760) { // 1 year in hours
      ...
      if (match.historicalSuccess !== undefined) {
        if (typeof match.historicalSuccess !== 'number' || match.historicalSuccess < 0 || match.historicalSuccess > 100) {
    ...
    try {
      ...
      if (request.analysisType && typeof request.analysisType === 'string') {
        ...
        if (!validTypes.includes(request.analysisType)) {
          errors.push(`Invalid analysis type: ${request.analysisType}. Must be one of: ${validTypes.join(', ')}`);
        }
      }
      ...
      if (request.confidenceThreshold !== undefined) {
        if (typeof request.confidenceThreshold !== 'number' || 
      ...
      if (request.includeHistorical !== undefined && typeof request.includeHistorical !== 'boolean') {
