...
export function useCurrencyFormatting() {
  const formatCurrency = useCallback((amount: number | null | undefined, decimals = 2) => {
  ...
  const formatBytes = useCallback((bytes: number | null | undefined): string => {
    // Handle null/undefined/NaN values
  ...
  const formatGrowthRate = useCallback((rate: number | null | undefined): string => {
