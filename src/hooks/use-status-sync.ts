...
export function useStatusSync() {
   ...
   */
  const getQueryKeysToInvalidate = useCallback((targetUserId?: string): (string | number)[][] => {
   ...
   */
  const invalidateStatusQueries = useCallback(async (targetUserId?: string) => {
   ...
   */
  const refetchStatusQueries = useCallback(async (targetUserId?: string) => {
    const queryKeys = getQueryKeysToInvalidate(targetUserId);
