...
class CostMonitor {
  ...
  async recordOperation(metrics: Omit<CostMetrics, 'timestamp' | 'estimatedCost'>): Promise<void> {
    const timestamp = new Date().toISOString();
    const estimatedCost = this.calculateCost(metrics.queryCount, metrics.duration, metrics.dataTransfer);
  ...
  private async checkOperationThresholds(metrics: CostMetrics): Promise<void> {
    ...
    } else if (metrics.dataTransfer >= this.costThresholds.dataTransfer.warning) {
  ...
  private createAlert(metrics: CostMetrics, severity: CostAlert['severity'], threshold: string): CostAlert {
    return {
      endpoint: metrics.endpoint,
  ...
  private async sendCostAlert(alert: CostAlert): Promise<void> {
    ...
    console.error(`${alertEmoji} [COST ALERT] ${alert.severity} threshold exceeded`, {
  ...
  private async checkGlobalCostLimits(): Promise<void> {
    ...
    if (currentUsage.totalCost > this.costThresholds.cost.critical) {
      await this.emergencyShutdown(`Total cost exceeded: $${currentUsage.totalCost.toFixed(2)}`);
    }
    
    if (currentUsage.queryCount > this.costThresholds.queries.critical * 100) { // 100x normal limit
      await this.emergencyShutdown(`Query count exceeded: ${currentUsage.queryCount} queries`);
    }
  }
  ...
  getEndpointMetrics(endpoint?: string) {
    ...
    for (const [ep, metrics] of this.endpointMetrics) {
      ...
      }), { totalQueries: 0, totalDuration: 0, totalDataTransfer: 0, totalCost: 0, operationCount: 0 });
 ...
 */
export function withCostMonitoring<T extends (...args: any[]) => Promise<NextResponse>>(
  handler: T,
  endpointName?: string
): T {
