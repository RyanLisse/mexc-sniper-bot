...
export const snipeTargetsRelations = relations(
  snipeTargets,
  ({ one, many }) => ({
    executionHistories: many(executionHistory),
    user: one(user, {
...
export const patternSimilarityCacheRelations = relations(
  patternSimilarityCache,
  ({ one }) => ({
    patternEmbedding_patternId1: one(patternEmbeddings, {
      ...
      relationName:
        "patternSimilarityCache_patternId1_patternEmbeddings_patternId",
    }),
    patternEmbedding_patternId2: one(patternEmbeddings, {
      fields: [patternSimilarityCache.patternId2],
      references: [patternEmbeddings.patternId],
      relationName:
        "patternSimilarityCache_patternId2_patternEmbeddings_patternId",
    }),
  })
);

export const patternEmbeddingsRelations = relations(
  patternEmbeddings,
  ({ many }) => ({
...
export const simulationSessionsRelations = relations(
  simulationSessions,
  ({ one, many }) => ({
    user: one(user, {
...
export const simulationTradesRelations = relations(
  simulationTrades,
  ({ one }) => ({
...
export const tradingStrategiesRelations = relations(
  tradingStrategies,
  ({ one, many }) => ({
    user: one(user, {
...
export const strategyTemplatesRelations = relations(
  strategyTemplates,
  ({ many }) => ({
    tradingStrategies: many(tradingStrategies),
  })
...
export const strategyConfigBackupsRelations = relations(
  strategyConfigBackups,
  ({ one }) => ({
...
export const strategyPhaseExecutionsRelations = relations(
  strategyPhaseExecutions,
  ({ one }) => ({
    user: one(user, {
...
export const transactionQueueRelations = relations(
  transactionQueue,
  ({ one }) => ({
...
export const transactionLocksRelations = relations(
  transactionLocks,
  ({ many }) => ({
