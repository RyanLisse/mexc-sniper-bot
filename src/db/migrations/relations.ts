...
import { user, account, alertRules, alertInstances, alertNotifications, notificationChannels, apiCredentials, executionHistory, snipeTargets, patternEmbeddings, patternSimilarityCache, positionSnapshots, session, simulationSessions, simulationTrades, tradingStrategies, strategyTemplates, strategyConfigBackups, strategyPerformanceMetrics, strategyPhaseExecutions, transactionLocks, transactionQueue, transactions, userPreferences } from "./schema";
...
export const strategyPerformanceMetricsRelations = relations(strategyPerformanceMetrics, ({one}) => ({
	user: one(user, {
