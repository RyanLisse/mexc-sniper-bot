...
function TestStatusComponent() {
  ...
  return (
    <div>
      ...
      <button onClick={refreshCredentials} data-testid="refresh-credentials">
        Refresh Credentials
      </button>
...
describe("StatusContext", () => {
  ...
  describe("Error Handling", () => {
    ...
    it("should clear errors when requested", async () => {
      ...
      (fetch as jest.Mock).mockRejectedValueOnce(new Error("Initial error")).mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({ success: true }),
      });
  ...
  describe("Status State Machine", () => {
    it("should follow correct status precedence", async () => {
      const scenarios = [
        {
          name: "Network down",
          mocks: [() => Promise.reject(new Error("Network error"))],
