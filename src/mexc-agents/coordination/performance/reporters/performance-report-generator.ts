...
export class PerformanceReportGenerator {
  ...
  private calculateSystemTrends(snapshots: SystemPerformanceSnapshot[]): {
    ...
    return {
      responseTime: {
        trend: Math.abs(responseTimeChange) < 5 ? "stable" : responseTimeChange < 0 ? "improving" : "degrading",
        change: Math.round(responseTimeChange),
      },
      throughput: {
        trend: Math.abs(throughputChange) < 5 ? "stable" : throughputChange > 0 ? "improving" : "degrading",
        change: Math.round(throughputChange),
      },
      errorRate: {
        trend: Math.abs(errorRateChange) < 5 ? "stable" : errorRateChange < 0 ? "improving" : "degrading",
  ...
  private generateRecommendations(
    ...
    const slowResponseAgents = agentMetrics.filter((m) => m.averageResponseTime > 3000);
    ...
    if (systemSnapshots.length > 0) {
      ...
      if (lastSnapshot.systemCpuUsage > 80) {
        recommendations.push("High CPU usage detected. Consider load balancing or scaling strategies.");
      }
      
      if (lastSnapshot.errorRate > 0.05) {
        recommendations.push("System error rate is elevated. Review error patterns and implement additional monitoring.");
      }
    }

    return recommendations.length > 0 ? recommendations : ["System performance is within acceptable parameters."];
  }
}