...
export class AgentManager {
  ...
  getAgentSummary(): {
    ...
    return {
      ...
      agentTypes: [
        // Core trading agents
        "mexc-api", "pattern-discovery", "calendar", "symbol-analysis", "strategy",
        // Safety agents
        "simulation", "risk-manager", "reconciliation", "error-recovery"
      ],
      initialized: !!(
        // Core agents
        this.mexcApiAgent &&
  ...
  async performComprehensiveSafetyCheck(): Promise<{
    ...
    try {
      const [simulationCheck, riskCheck, reconciliationCheck, errorRecoveryCheck] = await Promise.all([
      ...
      const criticalIssues = [simulationCheck, riskCheck, reconciliationCheck, errorRecoveryCheck]
