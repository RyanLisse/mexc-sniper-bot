...
export function ParameterMonitor() {
  ...
  useEffect(() => {
    const fetchParameters = async () => {
      try {
        ...
        const paramsResponse = await fetch('/api/tuning/parameters?includeDefinitions=true&includeHistory=true');
        const paramsData = await paramsResponse.json();
        
        // Convert to Parameter objects
        const parameterList: Parameter[] = Object.entries(paramsData.parameters).map(([name, value]) => {
          const definition = paramsData.definitions[name];
          const recentChanges = paramsData.changeHistory?.filter((change: ParameterChange) => 
          ...
          return {
            ...
            lastChanged: recentChanges[0]?.timestamp ? new Date(recentChanges[0].timestamp) : new Date(),
  ...
  const validateParameterValue = (parameter: Parameter, value: any): { valid: boolean; message?: string } => {
    ...
    if (parameter.constraints.options && !parameter.constraints.options.includes(value)) {
      return { valid: false, message: `Value must be one of: ${parameter.constraints.options.join(', ')}` };
    }
              ...
              <Card key={parameter.name} className="relative">
                ...
                <CardContent className="space-y-3">
                        ...
                        {parameter.type === 'boolean' ? (
                          ...
                          <Input
                            ...
                            onChange={(e) => setEditValue(
                              parameter.type === 'number' ? Number(e.target.value) : e.target.value
                            )}
                            className="w-32"
                  ...
                  <div className="flex items-center justify-between text-sm text-gray-500">
                    <span>Default: <code>{parameter.defaultValue?.toString()}</code></span>
                    <Button
                      size="sm"
                      variant="ghost"
                  ...
                  <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-600 mt-1">
                        <span className="line-through text-red-600">{change.oldValue?.toString()}</span>
