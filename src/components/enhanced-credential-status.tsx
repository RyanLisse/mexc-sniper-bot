...
import { useMexcConnectivity, type MexcConnectivityResult } from "@/src/hooks/use-mexc-data";
import { 
  CheckCircle, 
  XCircle, 
  AlertTriangle, 
...
export function EnhancedCredentialStatus({ showDetailsButton = true, className = "" }: EnhancedCredentialStatusProps) {
        ...
        <CardContent className="p-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <div className="text-sm text-muted-foreground">Verifying API credentials and connection</div>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">Unable to verify MEXC connection</div>
              </div>
            </div>
            <Button
              variant="outline"
  ...
  const getStatusConfig = () => {
    switch (connectivity.status) {
      case "fully_connected":
        return { variant: "default" as const, icon: CheckCircle, text: "Connected", color: "text-green-500" };
      case "no_credentials":
        return { variant: "secondary" as const, icon: Key, text: "No Credentials", color: "text-yellow-500" };
      case "invalid_credentials":
        return { variant: "destructive" as const, icon: XCircle, text: "Invalid Credentials", color: "text-red-500" };
      case "network_error":
        return { variant: "destructive" as const, icon: XCircle, text: "Network Error", color: "text-red-500" };
      default:
        return { variant: "secondary" as const, icon: AlertTriangle, text: "Unknown", color: "text-gray-500" };
    }
  };
...
function MainStatusRow({ connectivity, onRefresh }: { connectivity: MexcConnectivityResult; onRefresh: () => void }) {
  return (
    <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
      <div className="flex items-center space-x-4">
        <div className="flex items-center space-x-2">
          <div className={`w-3 h-3 rounded-full ${connectivity.connected ? 'bg-green-500' : 'bg-red-500'}`} />
          <span className="text-sm font-medium">
            {connectivity.connected ? 'Network Connected' : 'Network Disconnected'}
          </span>
        </div>
        <div className="flex items-center space-x-2">
          <Key className={`h-4 w-4 ${connectivity.hasCredentials ? 'text-green-500' : 'text-gray-400'}`} />
          <span className="text-sm">
            {connectivity.hasCredentials ? 'Credentials Found' : 'No Credentials'}
          </span>
        </div>
        <div className="flex items-center space-x-2">
          <Lock className={`h-4 w-4 ${connectivity.credentialsValid ? 'text-green-500' : 'text-red-500'}`} />
          <span className="text-sm">
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        ...
        <div>
          ...
          <div className={connectivity.hasEnvironmentCredentials ? "text-green-600" : "text-gray-400"}>
